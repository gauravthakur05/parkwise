...
- 🕒 Full parking history log (every entry & exit ever recorded)
- ⬇️ One-click **Download History as Excel (CSV)**
...
## 🕒 Parking History & Excel Export

Every time a vehicle is parked or exits, the C++ program appends one line to
`data/parking_history.txt` (this file is never overwritten, only added to,
so it becomes a permanent log).

- The dashboard's **Parking History** table shows every entry/exit, newest first.
- Click **⬇ Download Excel (CSV)** to download the full history as a `.csv`
  file — this opens directly in Microsoft Excel / Google Sheets, with columns
  for Event, Bay, Vehicle Number, Vehicle Type, Entry Time, Exit Time, and Fee.

Under the hood: `GET /api/history` returns the log as JSON for the table,
and `GET /api/history/download` returns the same data formatted as CSV text
with a `Content-Disposition: attachment` header so the browser saves it as a file.
